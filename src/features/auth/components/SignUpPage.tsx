...
function SignUp({ setSignedIn, setCurrentUser, setResetPassword, setMissedPassword }: SignUpProps) {
    const [name, setName] = useState('');
    const { showError } = useToast();
    const { mutateAsync: checkusername, isPending: isCheckRunning } = useCheckUserNameMutation();

    const check = async (name: string) => {
        const data: User = await checkusername(name);
        if (data?.is_blocked_by_admin) {
            showError('the selected user is blocked by admin');
        } else if (data?.account_status === 'password_expired') {
            showError('the selected user password is expired');
            setCurrentUser(data);
            setResetPassword(true);
        } else if (data?.account_status === 'password_missed') {
            showError('the selected user password is missed');
            setCurrentUser(data);
